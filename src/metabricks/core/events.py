...
def publish_event(
    *,
    ...
    Args:
        stage: Event stage identifier (e.g., "source.init", "sink.write")
        status: Event status (e.g., "started", "completed", "failed")
        duration_ms: Duration in milliseconds if applicable
        counts: Numeric metrics (e.g., record counts)
        details: Additional context-specific details
        error: Error information if status is "failed"
        source_asset: Asset identifier for source (e.g., "api://endpoint")
        sink_asset: Asset identifier for sink (e.g., "catalog.schema.table")
        source_system: Source system type for context
        source_dataset_id: Source dataset identifier for context
...
class FunctionalEvent:
    ...
    # Asset identifiers for lineage tracking
    source_asset: Optional[str] = None  # e.g., "api://api.data.org/v1/metadata"
    sink_asset: Optional[str] = None  # e.g., "main.test_data.metadata_raw"

    # Snapshot of execution context (optional, for correlation)
    environment: Optional[str] = None
    batch_id: Optional[str] = None
    pipeline_version: Optional[str] = None
    source_system: Optional[str] = None  # Source system type for asset context
    source_dataset_id: Optional[str] = None  # Source dataset identifier
