...
def test_pipeline_config_json_databricks_uc_to_uc(monkeypatch):
    from metabricks.orchestrator import run_pipeline

    cfg_path = Path(__file__).parent / "data" / "pipeline_databricks_uc_to_uc.json"
    ...
    # Sink side wrote to the UC delta table.
    assert env.context.metadata["sink_audit"]["status"] == "success"
    assert env.context.metadata["sink_audit"]["target_location"] == "main.dest_schema.dest_table"
    assert df.write.saved_table == "main.dest_schema.dest_table"
    assert df.write.format_value == "delta"
    assert df.write.mode_value == "overwrite"


def test_pipeline_json_rejects_invalid_shape():
    cfg_path = Path(__file__).parent / "data" / "pipeline_databricks_uc_to_uc.json"
    raw = cfg_path.read_text()

    # Basic sanity that we truly are using the client-facing Pydantic parsing.
    cfg = PipelineConfig.model_validate_json(raw)
    assert cfg.source.system_type == "databricks"
    assert cfg.sink is not None
    assert cfg.sink.system_type == "databricks"
